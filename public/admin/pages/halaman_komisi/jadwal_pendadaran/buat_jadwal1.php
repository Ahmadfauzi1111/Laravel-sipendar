...
$proses = "SELECT * FROM tb_jadwal";
$done = mysqli_query($conn , $proses);
while($cek=mysqli_fetch_array($done)){
  if ($tgl== $cek['tanggal'] && $shift == $cek['shift'] && $idruang == $cek['id_ruang']) {
    header("location:buat_jadwal.php?status=gagal");
  }
}
...
if (isset($_POST['simpan'])) {
  ...
  $sql = "INSERT INTO tb_jadwal VALUES ('','$tgl','$shift','$idruang','$idmhs','$dsn1','$dsn2','$dsn3','$dsn4')";
  $A = mysqli_query( $conn, $sql);

  $tahap = "UPDATE tb_mhs set tahap = 4 WHERE id_mahasiswa = $idmhs";
  mysqli_query( $conn, $tahap);
        ...
        <div class="col-xs-12">
          ...
          <div class="col-md-12">
              ...
              <!-- /.box-header -->
                <div class="box-body">
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-xs-6">
                        <select name="dsn1" class="form-control select2" data-placeholder="Pilih Dosen Penguji" required 
                         >
                            <option>
                      <?php 
                          $sq = mysqli_query($conn, "SELECT * FROM tb_mhs WHERE id_mahasiswa = $idmhs");
                          $hasil = mysqli_fetch_assoc($sq);

                          $dp = $hasil['dosen_pembimbing'];
                          $s1 = $hasil['skripsi1'];
                          $s2 = $hasil['skripsi2'];

                          $query = "SELECT * FROM tb_dsn WHERE bidang_keahlian = 'Ekologi' AND id_dosen <> '$s1' AND id_dosen <> '$s2' ORDER BY nama";
                      ...
                      <label for="" class="col-sm-2 control-label">Nama Dosen</label>
                      <div class="col-xs-6">
                        <select name="dsn2" class="form-control select2" data-placeholder="Pilih Dosen Penguji" required
                         >
                            <option>
                      <?php 
                          $query = "SELECT * FROM tb_dsn WHERE bidang_keahlian = 'Zoologi' AND id_dosen <> '$s1' AND id_dosen <> '$s2' ORDER BY nama";
                          $sql = mysqli_query($conn,$query);
                          while($row = mysqli_fetch_array($sql)){ 
                           ?>
                         <option value="<?php echo $row['id_dosen'] ?>">
                      ...
                      <label for="" class="col-sm-2 control-label">Nama Dosen</label>
                      <div class="col-xs-6">
                        <select name="dsn3" class="form-control select2" data-placeholder="Pilih Dosen Penguji" required
                         >
                            <option>
                      <?php 
                          $query = "SELECT * FROM tb_dsn WHERE bidang_keahlian = 'Molekuler' AND id_dosen <> '$s1' AND id_dosen <> '$s2' ORDER BY nama";
                          $sql = mysqli_query($conn,$query);
                          while($row = mysqli_fetch_array($sql)){ 
                           ?>
                         <option value="<?php echo $row['id_dosen'] ?>">
                      ...
                      <label for="" class="col-sm-2 control-label">Nama Dosen</label>
                      <div class="col-xs-6">
                        <select name="dsn4" class="form-control select2" data-placeholder="Pilih Dosen Penguji" required
                         >
                            <option>
                      <?php 
                          $query = "SELECT * FROM tb_dsn WHERE bidang_keahlian = 'Botani' AND id_dosen <> '$s1' AND id_dosen <> '$s2' ORDER BY nama";
                          $sql = mysqli_query($conn,$query);
                          while($row = mysqli_fetch_array($sql)){ 
                           ?>
                         <option value="<?php echo $row['id_dosen'] ?>">
           ...
           <div class="col-xs-12 text-center">
                   ...
                   <button name="simpan" type="submit" class="btn btn-primary">Selesai</button>
                   </div>
                  </div>
        </form>
      </div>
